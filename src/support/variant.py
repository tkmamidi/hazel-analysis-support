...
def variant():
    ...
    def load_data():
        ...
        gene2hpo = {}
        #gene2hpo_assoc = {}
        r = requests.get("http://purl.obolibrary.org/obo/hp/hpoa/genes_to_phenotype.txt", stream=True)
        #r.text
    ...
    if st.button('submit') and len(terms) != 0:
        ...
        col2.write(f"Hazel score = {score}")
        ...
        all_associations = {key + " : " + id_to_name[key] for key in list(set(terms) & set(gene2hpo[gene]+gene2hpo_assoc))}
        col2.write("\n\nDirect associations:")
